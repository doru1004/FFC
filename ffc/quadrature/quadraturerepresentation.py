...
def sort_integrals(integrals, default_quadrature_degree, default_quadrature_rule):
    ...
    ffc_assert(all(space == itg.space() for itg in integrals),
               "Expecting only integrals of the same space type.")
    ...
    for key, integrands in list(sorted_integrands.items()):
        ...
        sorted_integrals[key] = Integral(integrand, integral_type, space, domain, subdomain_id, {}, None)
